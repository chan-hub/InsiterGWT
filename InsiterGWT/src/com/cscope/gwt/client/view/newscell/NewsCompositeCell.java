 ...
 * to support scroll position saving and restoration, by checking
 * the NewsTitleCell of event source - in the click event handler
 *
 */
public class NewsCompositeCell {
   ...
   * 1) Logo image
   * 2) Title in bold
   * 3) For non-mobile screen, Article entry in light
   * 4) NameOfAuthor, Counter & Time-elapsed
   //* 4) NameOfSource, Counter & Time-elapsed
   * 5) Subjects
   * 6) Item index
   * @return
   */
  public NewsCompositeCell(final EventBus eBus) {
    // Cell components of the CompositeCell to build
    this.cellComponents = new ArrayList<HasCell<NewsContents, ?>>();
  
    // Put Logo image (if image is available)
    cellComponents.add(new HasCell<NewsContents, String>() {
      private LogoImgCell cell = new LogoImgCell();
    ...
    cellComponents.add(new HasCell<NewsContents, String>() {
      ...
      public String getValue(NewsContents nct) {
        // Get the source URL
        String srcUrl = nct.srcUrl;
        // Use 2 backslashes to combine the title & URL
        if (nct.srcTitle!=null && !nct.srcTitle.isEmpty()) {
          return nct.srcTitle + titleAndUrlBinder + srcUrl;
        } else {
          return nct.name + titleAndUrlBinder + srcUrl;
        }
      }
    });
    ...
    // Create a composite cell with All of the above cell components
